...
try:
    from llama_cpp import Llama          # real import test
    LLAMA_AVAILABLE = True
except ImportError:
    LLAMA_AVAILABLE = False
    print("[Warning] llama_cpp not found – local backend disabled.")
...
class MiniTriniApp:
    ...
    # ==================================================================
    # BACKEND SEND — restored from working MiniTrini_v11_dev.py
    # ==================================================================
    ...
    def send_to_backend(self, prompt_text):
        ...
        if self.backend_mode == "local":
            ...
            try:
                cfg = self._get_generation_config()
                result = self.llm(
                    prompt=final_prompt,
        ...
        elif self.backend_mode == "ollama":
            try:
                ollama_model = getattr(self, "ollama_model_name", None) or self.model_var.get()
                ...
                llm_text = _re.sub(r"<think>.*?</think>", "", raw, flags=_re.DOTALL).strip()
                llm_text = llm_text if llm_text else raw.strip()
                print("CLEANED:", repr(llm_text))
                code = self.extract_code_block(llm_text)
                if code:
                    try:
                        from TrinityCanvas import send_to_canvas
    ...
    def _toggle_speed_profile(self):
        if self.speed_profile == "build":
            print("[MiniTrini] BUILD profile active – model call skipped.")
            self.speed_profile = "runtime"
    ...
    def process_user_input(self, text):
        print(f"[DEBUG] process_user_input HIT: {repr(text)}")
    ...
    def _restore_active_file(self, mod_dir, module_name):
        ...
        # Normal chat + non-web tools — direct backend call (restored)
        agent_name = self.state.get("agent") or "Trini"
        tool_name  = self.state.get("tool")
    ...
    def _run_chat_async(self, user_text: str):
        ...
        try:
            ...
            print("[MiniTrini] engine returned:", repr(response_text))   # ← add this

            if response_text is None:
                raise RuntimeError("No response – is a model loaded?")

        except Exception as e:
            import traceback
            print("[_run_chat_async ERROR]", str(e), file=sys.stderr)
            traceback.print_exc()
            self.root.after(
                0,
                lambda err=str(e): (
        ...
        def _ui_update():
            print("[MiniTrini] pushing to widget:", repr(response_text))
...
# ==================================================================
    ...
    def toggle_backend(self):
        ...
        if hasattr(self, "model_menu"):
            values = self.get_model_list() if self.backend_mode == "local" else self.get_ollama_models()
            self.model_menu["values"] = values
    ...
    def load_selected_model(self, event=None):
        ...
        if self.backend_mode == "ollama":
            ...
            self.console_print(f"[Ollama] Selected model: {model_name}")
            print("[MiniTrini] selected ollama model:", model_name)
            return
        ...
        try:
            ...
            print("[MiniTrini] selected model:", model_name)
            print("[MiniTrini] model path exists:", model_path.exists())

            if not model_path.exists():
                self.console_print(f"[Local] Model not found: {model_path}")
                self.local_model_loaded = False
                return
            ...
            self.console_print(f"[Local] Loaded model: {model_name}")

        except Exception as e:
            print("MODEL LOAD ERROR:", e)
            self.console_print(f"[Local] Model load error: {e}")
            self.local_model_loaded = False
...
if __name__ == "__main__":
    # ── Integrity Shield ──────────────────────────────────────────
    try:
        from minitrini_guard import startup_check
        startup_check()
    except Exception as _guard_err:
        print(f"[MiniTrini Guard] Could not run integrity check: {_guard_err}")
    # ─────────────────────────────────────────────────────────────
    root = TkinterDnD.Tk()
    ...
    if app.model_menu["values"]:
        app.model_var.set(app.model_menu["values"][0])  # pick the first .gguf found
        app.load_local_model()                          # actually instantiate Llama
    ...
    app.model_menu.bind("<<ComboboxSelected>>", app.load_selected_model)
